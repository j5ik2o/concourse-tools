...
RN=$(helm list --deployed --failed --date --reverse | grep -E "${APP_NAME}.*SNAPSHOT" | head -n 1 | cut -f 1)

echo "RELEASE NAME = ${RN}"
echo "RELEASE NAME = ${RN}" >> ../../notify-message/text

if [ "${RN}" != "" ]; then
  helm upgrade --debug ${RN} local/${APP_NAME}-${APP_VERSION}.tgz --set imageRegistry=${IMAGE_REGISTRY},appVersion=${APP_VERSION},version=${APP_VERSION} > /dev/null
  ...
  helm install --debug local/${APP_NAME}-${APP_VERSION}.tgz --namespace ${KUBE_NAMESPACE} --set imageRegistry=${IMAGE_REGISTRY},appVersion=${APP_VERSION},version=${APP_VERSION} > /dev/null
